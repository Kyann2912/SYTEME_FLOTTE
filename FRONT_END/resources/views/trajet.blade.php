    ...
    <div class="container-fluid">
        <div class="row justify-content-between mt-5">
            ...
            <div class="col-md-5 p-4" style="background-color: #f8f9fa; border-radius: 10px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);">
                <h1 style="color: #007BFF;"><i class="fas fa-route" style="color: #007BFF;"></i> Ajouter un Trajet</h1>
                <form action="{{ route('trajets.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="distance" id="distance" required min="1">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="idchauffeur" class="form-control" id="chauffeur" required>
                            <option value="">Sélectionnez un chauffeur</option>
                            <!-- Affichage des chauffeurs -->
                            @foreach($chauffeurs as $chauffeur)
                                <option value="{{ $chauffeur['id'] }}">{{ $chauffeur['nom'] }} {{ $chauffeur['prenoms'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="idvehicule" class="form-control" id="vehicule" required>
                            <option value="">Sélectionnez un véhicule</option>
                            <!-- Affichage des véhicules -->
                            @foreach($vehicules as $vehicule)
                                <option value="{{ $vehicule['id'] }}">{{ $vehicule['immatriculation'] }} {{ $vehicule['model'] }}</option>
                            @endforeach
                        </select>
                    </div>

                    <button type="submit" class="btn btn-success w-100">Ajouter le Trajet</button>
                </form>
            </div>
